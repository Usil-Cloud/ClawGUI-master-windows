...
def check_system_requirements(
    ...
    # Windows uses pyautogui (local) or the Windows Agent Server (remote).
    # No external toolchain (ADB / HDC / WDA) is needed.
    if device_type == DeviceType.WINDOWS:
        print("✅ Windows mode — no external device toolchain required.")
        print("   Local mode  : pyautogui controls this PC directly.")
        print("   Remote mode : start windows_agent_server.py on the target PC,")
        print("                 then pass --device-id <ip>:<port> (default port 7860).")
        print("-" * 50)
...
def _handle_windows_device_commands(args) -> bool:
    """
    Handle Windows-specific device CLI commands.

    Windows uses the Windows Agent Server (WAS) over HTTP instead of
    ADB/HDC/iOS.  --connect verifies the WAS is reachable; --list-devices
    shows info about this machine (local mode) or a remote WAS.

    Returns:
        True if a device command was handled (should exit), False otherwise.
    """
    from phone_agent.windows.connection import WindowsConnection, list_devices as win_list_devices

    if args.list_devices:
        devices = win_list_devices()
        print("Windows devices:")
        print("-" * 60)
        for d in devices:
            print(f"  ✓ {d.machine} (local) — {d.os}, {d.screen_width}×{d.screen_height}")
        print()
        print("To use a remote Windows PC, start windows_agent_server.py on that")
        print("machine and connect with: python main.py --device-type windows --connect <ip>:7860")
        return True

    if args.connect:
        print(f"Connecting to Windows Agent Server at {args.connect}...")
    ...
    if args.disconnect:
        print("Windows Agent Server connections are stateless — nothing to disconnect.")
        return True
...
def handle_device_commands(args) -> bool:
    ...
    device_type = _dtype_map.get(args.device_type, DeviceType.ADB)

    # Handle iOS-specific commands
    if device_type == DeviceType.IOS:
        return handle_ios_device_commands(args)

    # Handle Windows-specific commands (WAS uses HTTP, not ADB/HDC)
    if device_type == DeviceType.WINDOWS:
        return _handle_windows_device_commands(args)
