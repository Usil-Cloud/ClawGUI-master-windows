...
class PhoneAgent:
    ...
    def __init__(
        ...
        if self._model_type == ModelType.UITARS:
            ...
            # Use the Windows-extended handler when running against a Windows target
            if get_device_factory().device_type.value == "windows":
                from phone_agent.actions.handler_guiowl_windows import GUIOwlWindowsActionHandler
                self._specialized_handler = GUIOwlWindowsActionHandler(
                    device_id=self.agent_config.device_id,
                    confirmation_callback=confirmation_callback,
                    takeover_callback=takeover_callback,
                )
        ...
        # Initialize step memory for GUIOWL on Windows (verl-agent rolling-window pattern).
        # Complements long-term MemoryManager: StepMemory tracks within-episode context;
        # MemoryManager handles cross-session preferences and task history.
        self._step_memory = None
        if (self._model_type == ModelType.GUIOWL
                and get_device_factory().device_type.value == "windows"):
            try:
                from phone_agent.memory.step_memory import StepMemory
                self._step_memory = StepMemory()
                if self.agent_config.verbose:
                    print("📋 Step memory (rolling window) enabled for Windows GUI-Owl")
            except Exception as e:
                if self.agent_config.verbose:
                    print(f"⚠️ Step memory init failed: {e}")
    ...
    def run(self, task: str) -> str:
        ...
        # Reset step memory at the start of each new task
        if self._step_memory is not None:
            self._step_memory.reset()
    ...
    def _execute_step(
        ...
        if self._model_type in (ModelType.QWENVL, ModelType.GUIOWL):
            ...
            if action_description and hasattr(self._adapter, 'add_history'):
                ...
                # If StepMemory is active (Windows), sync a bounded rolling window
                # back into the adapter's history so the prompt stays bounded.
                if self._step_memory is not None:
                    self._step_memory.store(
                        observation=current_app,
                        action=action_description,
                        success=result.success,
                    )
                    # Replace adapter history with the last 10 steps (rolling window)
                    self._adapter._action_history = self._step_memory.fetch_as_list(
                        history_length=10
                    )
        elif self._model_type == ModelType.MAIUI:
